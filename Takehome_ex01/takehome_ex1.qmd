...
```{r}
fips_wdmp <- order(total_trips_by_grid_wdmp_sf$grid_id)
...
```{r}
fips_wdap <- order(total_trips_by_grid_wdap_sf$grid_id)
...
```{r}
fips_hmp <- order(total_trips_by_grid_hmp_sf$grid_id)
...
```{r}
fips_hep <- order(total_trips_by_grid_hep_sf$grid_id)
...
Append the local Moran's I dataframe (i.e. localMI) onto total_trips_by_grid patialPolygonDataFrame.
...
Both the local Moran's I values map and its corresponding p-values map next to each other for effective interpretation

#### **Weekday Morning Peak 6am-9am**

```{r}
total_trips_wdmp <- qtm(total_trips_by_grid_wdmp_sf, "total_trips")

total_trips_by_grid_wdmp_sf.localMI_wdmp$quadrant_wdmp <- quadrant_wdmp

colors <- c("#ffffff", "#2c7bb6", "#abd9e9", "#fdae61", "#d7191c")
clusters <- c("insignificant", "low-low", "low-high", "high-low", "high-high")

LISAmap_wdmp <- tm_shape(total_trips_by_grid_wdmp_sf.localMI_wdmp$quadrant_wdmp) +
