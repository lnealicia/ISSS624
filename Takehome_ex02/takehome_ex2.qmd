...
Note: Some postal codes are missing from hdb.csv. Manually update:

```{r}
hdb <- hdb %>%
  mutate(
    postal = ifelse(addr == "115 BISHAN STREET 12", 570115, postal),
    postal = ifelse(addr == "12 KAMPONG ARANG ROAD", 431012, postal),
    postal = ifelse(addr == "12 KAMPONG ARANG ROAD", 431012, postal),
  )
```
...
Note: hdb is a tibble dataframe object class
