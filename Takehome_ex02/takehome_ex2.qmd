...
Remove duplicates, select relevant columns, rename columns
...
for(address in addresses){
  query <- list('searchVal' = address, 'returnGeom' = 'Y', 'getAddrDetails' = 'Y', 'pageNum' = '1')
  res <- GET(url, query = query)
  
  if (content(res)$found != 0) {
    found <- rbind(found, data.frame(content(res))[4:13])
  } else {
      not_found <- rbind(not_found, data.frame(address))
    }
  }
```
